class FactsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @fact.save
        format.html { redirect_to fact_url(@fact), notice: "Fact was successfully created." }
  ...
  def update
    respond_to do |format|
      if @fact.update(fact_params)
        format.html { redirect_to fact_url(@fact), notice: "Fact was successfully updated." }
  ...
  private
    ...
    def fact_params
      params.require(:fact).permit(:character, :droid, :planet, :quote)
    end
end
